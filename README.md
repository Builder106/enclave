...
**Trial 04 — Hosted open-weights · openai/gpt-oss-120b via Groq · same 50 docs**

> **100% parse rate, 98.3% field accuracy, 78.0% exact match, code-match F1 99.6%, anomaly-detection F1 95.2% — at $0.0008/doc metered list price ($0 billed on the free tier) and 97,303 bytes of PHI egress.** Forty times the local model's parameters closes nearly every gap: field accuracy clears the rules floor (98.3 vs 95.0), code matching and anomaly detection beat it outright (99.6 / 95.2 vs 98.3 / 90.0) — only exact match still trails the regexes (78.0 vs 84.0). The p50 latency (11.0 s) is free-tier rate-limit pacing, not the model: unthrottled calls return in ~1.8 s. And the egress column finally shows a number — **97 KB of (synthetic) PHI left the machine to buy that accuracy.** That's the thesis in one row: scale buys accuracy, and the price is denominated in bytes as much as dollars. (Model choice: gpt-oss-120b is the json_schema-capable open-weights option on Groq — llama-3.3-70b and qwen3-32b both rejected the schema.) Reproduce: `GROQ_API_KEY=... pnpm measure --provider groq --seed 1`.

**Trial 03 (Bedrock / Claude Haiku 4.5) — pending, honestly.** Blocked by AWS's new-account ramp: fresh accounts get Claude's daily-token quota pinned near zero, and it is not self-service adjustable. A launchd job (`scripts/trial03-cron.sh`) retries daily with `--resume`, measuring only still-unmeasured docs each reset; quota throttles are excluded from metrics as infrastructure noise, never counted as model failures. The trial runs ONCE — the job removes its own schedule the moment coverage reaches 50/50.
